  ...
  return (
    <div className="min-h-screen bg-[#080c16] px-4 py-6 pb-24">
      <div className="max-w-4xl mx-auto">
        ...
        {showGuide && <div className="mt-5 rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5"><div className="flex items-start justify-between gap-4"><div className="flex gap-3"><BookOpen className="text-violet-300 mt-0.5" size={20} /><div><h2 className="font-semibold text-white">Simple guide for Follow-Up workers</h2><ol className="mt-3 space-y-2 text-sm text-slate-300 list-decimal pl-5"><li>Open <b>Follow-Up</b> and select the person you need to contact.</li><li>Tap <b>Call</b> to start the call, then record the outcome after the conversation.</li><li>Use <b>WhatsApp / SMS</b> when a message is more appropriate. Keep messages short and personal.</li><li>After a call, always save the result and set the next follow-up date if another contact is needed.</li><li>If the balance warning is red, tell an admin before making more calls.</li></ol></div></div><button onClick={() => setShowGuide(false)} className="text-xs text-slate-400 hover:text-white">Hide</button></div></div>}
        ...
        <div className="grid md:grid-cols-2 gap-4 mt-5">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 flex gap-2"><Button onClick={() => void syncBalance()} disabled={syncing} className="flex-1"><RefreshCw size={14} className="mr-2" /> Check balance</Button><a href="https://account.africastalking.com/" target="_blank" rel="noreferrer" className="flex-1"><Button variant="secondary" className="w-full"><WalletCards size={14} className="mr-2" /> Recharge</Button></a></div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] p-3"><div><div className="text-sm text-white">Messaging is {settings.messaging_enabled ? "ON" : "OFF"}</div><div className="text-xs text-slate-500 mt-1">{settings.sender_id ? "Sender ID: " + settings.sender_id : "Sender ID not configured"}</div></div><button disabled={saving} onClick={() => void saveSettings({ messaging_enabled: !settings.messaging_enabled })} className={settings.messaging_enabled ? "rounded-full bg-emerald-500/20 px-3 py-1.5 text-xs text-emerald-300" : "rounded-full bg-white/10 px-3 py-1.5 text-xs text-slate-300"}>{settings.messaging_enabled ? "Turn off" : "Turn on"}</button></div>
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2 text-slate-300"><Settings2 size={17} /><span className="font-semibold">Admin settings</span></div><p className="text-xs text-slate-500 mt-1">Only admins should change these values.</p><div className="grid md:grid-cols-3 gap-3 mt-4"><Input label="Voice number" value={settings.voice_number ?? ""} onChange={(e) => setSettings({ ...settings, voice_number: e.target.value })} placeholder="+234..." /><Input label="Sender ID" value={settings.sender_id ?? ""} onChange={(e) => setSettings({ ...settings, sender_id: e.target.value })} placeholder="ShepherdsHill" /><Input label="Voice rate / minute (NGN)" type="number" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="Enter current provider rate" /></div><div className="grid md:grid-cols-3 gap-3 mt-3"><Input label="Low balance alert (NGN)" type="number" value={threshold} onChange={(e) => setThreshold(e.target.value)} /><div className="md:col-span-2 flex items-end"><Button loading={saving} onClick={() => void saveSettings({ voice_number: settings.voice_number || null, sender_id: settings.sender_id || null, voice_rate_per_minute: rate ? Number(rate) : null, low_balance_threshold: threshold ? Number(threshold) : 5000 })}>Save settings</Button></div></div></div>

        <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5"><h2 className="font-semibold text-white">How to recharge</h2><div className="mt-3 space-y-2 text-sm text-slate-300"><p><b>1.</b> Tap <b>Recharge</b> above.</p><p><b>2.</b> In the provider dashboard, open <b>Billing → Top Up</b>.</p><p><b>3.</b> Choose the country and payment method, then complete the payment.</p><p><b>4.</b> Return here and tap <b>Check balance</b>.</p></div><p className="text-xs text-slate-500 mt-3">We deliberately do not store card details in Shepherd's Hill.</p><a className="inline-flex items-center gap-2 text-xs text-violet-300 mt-3 hover:text-white" href="https://help.africastalking.com/en/articles/1981959-how-do-i-top-up-my-account" target="_blank" rel="noreferrer">View official recharge guide <ExternalLink size={13} /></a></div>
