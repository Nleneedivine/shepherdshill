        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2 text-slate-300"><Settings2 size={17} /><span className="font-semibold">Admin settings</span></div>
          <div className="mt-4 grid md:grid-cols-2 gap-3">
            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 flex items-center justify-between">
              <div><div className="text-sm text-white">System calling</div><div className="text-xs text-slate-500 mt-1">Workers can place recorded calls through Africa's Talking.</div></div>
            ...
            <div className="rounded-xl border border-amber-400/10 bg-amber-500/[0.04] p-3 text-xs text-slate-400">Keep calling OFF until the Africa's Talking number, credentials and callback URL are configured.</div>
          </div><p className="text-xs text-slate-500 mt-1">Only admins should change these values.</p><div className="grid md:grid-cols-3 gap-3 mt-4"><Input label="Voice number" value={settings.voice_number ?? ""} onChange={(e) => setSettings({ ...settings, voice_number: e.target.value })} placeholder="+234..." /><Input label="Sender ID" value={settings.sender_id ?? ""} onChange={(e) => setSettings({ ...settings, sender_id: e.target.value })} placeholder="ShepherdsHill" /><Input label="Voice rate / minute (NGN)" type="number" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="Enter current provider rate" /></div><div className="grid md:grid-cols-3 gap-3 mt-3"><Input label="Low balance alert (NGN)" type="number" value={threshold} onChange={(e) => setThreshold(e.target.value)} /><div className="md:col-span-2 flex items-end"><Button loading={saving} onClick={() => void saveSettings({ voice_number: settings.voice_number || null, sender_id: settings.sender_id || null, voice_rate_per_minute: rate ? Number(rate) : null, low_balance_threshold: threshold ? Number(threshold) : 5000 })}>Save settings</Button></div></div></div>
